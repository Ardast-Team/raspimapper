...
def server_error(request, template_name='500.html'):
    ...
    return http.HttpResponseServerError(temp.render(template.Context({'exc_info':exc_info})))
...
def reports(request,*kw,**kwargs):
    if request.method == 'GET':
        if 'select' in request.GET:             #from menu:select->reports
            ...
            return render(request, form.template, {'form': form})    #go to the SelectReports form
...
def incoming(request,*kw,**kwargs):
    if request.method == 'GET':
        if 'select' in request.GET:             #from menu:select->incoming
            ...
            return render(request, form.template, {'form': form})    #go to the SelectIncoming form
    ...
    if request.method == 'GET':
        ...
        if 'fromselect' in request.POST:        #from SelectOutgoing form
            ...
            if not formin.is_valid():
                ...
                #select all objects with parameters and set retransmit
                query = models.Transaction.objects.filter(status=EXTERNOUT)
...
def document(request,*kw,**kwargs):
    ...
    query = models.Transaction.objects.filter(Q(status=SPLITUP)|Q(status=TRANSLATED))
    pquery = viewlib.filterquery(query,cleaned_data)
...
def process(request,*kw,**kwargs):
    ...
    query = models.Transaction.objects.filter(status=PROCESS,statust=ERROR)
    pquery = viewlib.filterquery(query,cleaned_data)
...
def detail(request,*kw,**kwargs):
    ...
    if request.method == 'GET':
        ...
        return render(request,'bots/detail.html',{'detaillist':detaillist,'rootta':rootta})
...
def confirm(request,*kw,**kwargs):
    ...
    query = models.Transaction.objects.filter(confirmasked=True)
    pquery = viewlib.filterquery(query,cleaned_data)
...
def filer(request,*kw,**kwargs):
    ...
    if request.method == 'GET':
        try:
            ...
            return render(request,'bots/filer.html',{'error_content': 'No such file.'})
    ...
    if request.method == 'GET':
        try:
            ...
            if botsglobal.ini.get('directories','usersys') in src and src.endswith('.py'): # only python source in usersys!
                ...
                return render(request,'bots/srcfiler.html',{'src':src, 'html_source':html_source})
            else:
                return render(request,'bots/srcfiler.html',{'error_content': 'File %s not allowed.' %src})
        except:
            return render(request,'bots/srcfiler.html',{'error_content': 'No such file.'})
...
def logfiler(request,*kw,**kwargs):
    ...
    if request.method == 'GET':
        ...
        if 'action' in request.GET and request.GET['action'] == 'download':
            response = http.HttpResponse(content_type='text/log')
            ...
            return render(request,'bots/logfiler.html',{'log':log, 'logdata':logdata, 'logfiles':logfiles})
...
def plugin(request,*kw,**kwargs):
    if request.method == 'GET':
        ...
        return redirect('/bots/home')

def plugin_index(request,*kw,**kwargs):
    if request.method == 'GET':
        return render(request,'bots/plugin_index.html')
    else:
        if 'submit' in request.POST:        #coming from ViewIncoming, go to outgoing form using same criteria
            #write backup plugin first
            plugout_backup_core(request,*kw,**kwargs)
...
def send_job_to_djangoq(python_executable_path,botsengine_path,environment):
    #run engine
    lijst = [python_executable_path,botsengine_path,environment]
    try:
        terug = subprocess.Popen(lijst).pid
        print('Run bots-engine via DJANGO Q: "%s".'%terug)
    except Exception as msg:
        print('Errors while trying to run bots-engine via DJANGO Q: "%s".'%msg)

def result_from_djangoq(task):
    #messages.add_message(request, messages.INFO, task_result)
    botsglobal.logger.info(print(task.result))
    
    #return redirect('/bots/add_async_message',kwargs={'task_id':task})
    #request = task.kwargs.get('kwargs').get('request')

def test_djangod():
    print("test....")   

def add_async_message(request,*kw,**kwargs):
    task_id = kwargs.get('task_id')
    #messages.add_message(request, messages.INFO, ''.join(("DJANGO Q RES:", str(task_id))))
    return redirect('/bots/home') 
...
def runengine(request,*kw,**kwargs):
    ...
    if request.method == 'GET':
        ...
        if botsglobal.ini.getboolean('jobqueue','enabled',False):   #run bots-engine via jobqueue-server; reports back if job is queued
            ...
            # #Manage with Django Q
            # task_id = async_task(
            #     "bots.views.send_job_to_djangoq",
            #     python_executable_path, 
            #     botsengine_path, 
            #     environment,
            #     sync=True,
            #     hook='bots.views.result_from_djangoq',
            #     #kwargs={"request": "this_request"}
            #     )

            # messages.add_message(request, messages.INFO, ''.join(("DJANGO Q:", task_id)))
            # messages.add_message(request, messages.INFO, ''.join(("DJANGO Q RES:", str(result(task_id)))))
            # #task_id = async_task("bots.views.test_djangod")
            # #task_result = result(task_id,200)
            # #if task_result is not None:
            # #     messages.add_message(request, messages.INFO, task_result)
            # #     botsglobal.logger.info(task_result)
            pass
