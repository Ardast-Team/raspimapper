    ...
    def get_parent(ta_object):
        ...
        if ta_object.parent:
            if ta_object.parent not in donelijst:   #search via parent
                yield models.Transaction.objects.get(idta=ta_object.parent)
        else:
            for parent in models.Transaction.objects.filter(idta__range=(ta_object.script,ta_object.idta),child=ta_object.idta):
...
def trace_document(pquery):
    ...
    def trace_forward(ta_object):
        ...
        if ta_object.child:
            child = models.Transaction.objects.get(idta=ta_object.child)
        else:
            try:
                child = models.Transaction.objects.filter(parent=ta_object.idta)[0]
            except IndexError:
    ...
    def trace_back(ta_object):
        ...
        if ta_object.parent:
            parent = models.Transaction.objects.get(idta=ta_object.parent)
        else:
            try:
                parent = models.Transaction.objects.filter(idta__range=(ta_object.script,ta_object.idta),child=ta_object.idta)[0]   #just get one parent
            except IndexError:
                return    #no result, return
...
def gettrace(ta_object):
    ...
    if ta_object.child:  #has a explicit child
        ta_object.talijst = [models.Transaction.objects.get(idta=ta_object.child)]
    else:   #search in ta_object-table who is reffering to ta_object
        ta_object.talijst = list(models.Transaction.objects.filter(parent=ta_object.idta))
...
def delete_from_ta(ta_object):
    ...
    def gather_tas_before_merge(ta_object):
        ...
        if ta_object.child:  #has a explicit child
            tmp_list = [models.Transaction.objects.get(idta=ta_object.child)]
        else:   #search in ta_object-table who is reffering to ta_object
            tmp_list = list(models.Transaction.objects.filter(parent=ta_object.idta))
    ...
    for ta_merge in tas_merge:  #for each MERGE ta:
        ...
        for includedta in models.Transaction.objects.filter(idta__range=(ta_merge.script,ta_merge.idta),child=ta_merge.idta):    #select all db-ta_object's included in MERGED ta_object
    ...
    def newbranche(ta_object,level=0):
        def dota(ta_object, isfirststep = False):
            ...
            if isfirststep and level:
                ...
                ta_object.ind = levelindicator
            #~ ta_object.action = models.Transaction.objects.only('filename').get(idta=ta_object.script)
