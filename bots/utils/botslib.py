...
from django.db.models import Count, Q, Value, F, Func, ExpressionWrapper, Case, When, IntegerField
'''
...
class _Transaction(object):
    ...
    def update(self,**ta_info):
        ...
        setdict = dict((key,value) for key,value in ta_info.items() if key in self.filterlist)
        ta_info['selfid'] = self.idta
        #changeq('''UPDATE ta SET '''+setstring+ ''' WHERE idta=%(selfid)s''', ta_info)
        queryset = Transaction.objects.filter(idta=self.idta).update(**setdict) 
       # print('_Transaction update:')
       # print(queryset.query)
        
    def delete(self):
        '''Deletes current transaction '''
        queryset = Transaction.objects.filter(idta=self.idta).delete()
       # print('_Transaction delete:')
       # print(queryset.query)
        #changeq('''DELETE FROM ta WHERE idta=%(idta)s''', {'idta':self.idta})

    def deletechildren(self):
        self.deleteonlychildren_core(self.idta)

    def deleteonlychildren_core(self,idta):
        queryset = Transaction.objects.filter(parent=self.idta).delete()
       # print('_Transaction deleteonlychildren_core:')
       # print(queryset.query)
    ...
    def copyta(self,status,**ta_info):
        ...
        oldta = Transaction.objects.filter(idta=self.idta).values('idta',
                                                                  'script',
                                                                  'status',
                                                                  'parent',
                                                                  'frompartner',
                                                                  ...
                                                                  'cc')
        #print(queryset.query)
       # print('_Transaction copyta oldta:')
       # print(oldta.query)
        ...
        newta = Transaction.objects.create( script=oldta[0]['script'],
                                            ...
                                            fromchannel=oldta[0]['fromchannel'],
                                            tochannel=oldta[0]['tochannel'],
                                            editype=oldta[0]['editype'],
                                            messagetype=oldta[0]['messagetype'],
                                            alt=oldta[0]['alt'],
                                            merge=oldta[0]['merge'],
                                            testindicator=oldta[0]['testindicator'],
                                            reference=oldta[0]['reference'],
                                            frommail=oldta[0]['frommail'],
                                            tomail=oldta[0]['tomail'],
                                            charset=oldta[0]['charset'],
                                            contenttype=oldta[0]['contenttype'],
                                            filename=oldta[0]['filename'],
                                            idroute=oldta[0]['idroute'],
                                            nrmessages=oldta[0]['nrmessages'],
                                            botskey=oldta[0]['botskey'],
                                            envelope=oldta[0]['envelope'],
                                            rsrv3=oldta[0]['rsrv3'],
                                            cc=oldta[0]['cc']
                                             )
       # print('_Transaction copyta newta:')
       # print(newta.query)
        newidta=getattr(newta,'idta')
...
class NewTransaction(_Transaction):
    ...
    def __init__(self,**ta_info):
        ...
        #self.idta = insertta('''INSERT INTO ta (''' + namesstring + ''')  VALUES   (''' + varsstring + ''')''', updatedict)
        queryset = Transaction.objects.create(**updatedict)
        self.idta = queryset.pk
       # print('NewTransaction __init__:')
       # print(queryset.query)
...
def addinfo(change,where):
    ...
    return addinfocore_noquery(change=change,where=where,wheredict=wheredict)

def updateinfocore(change,where,wherestring,**wheredict):
    ''' update info in ta's.
...
def unique_runcounter(domain,updatewith=None):
    ...
    number = getattr(botsglobal,str(domain),0)
    if updatewith is None:
        number += 1
        updatewith = number
        if updatewith > MAXINT:
            updatewith = 0
    setattr(botsglobal,str(domain),updatewith)
    return number

def unique(domain,updatewith=None):
    ...
    if botsglobal.ini.getboolean('acceptance','runacceptancetest',False):
        return unique_runcounter(domain)
    else:
        try:
            queryset = Unique.objects.filter(domain=domain).values('number')
            ...
            if updatewith is None:
                ...
                if updatewith > MAXINT:
                    updatewith = 0
            queryset = Unique.objects.filter(domain=domain).update(number=updatewith)
        except TypeError: #if domein does not exist, cursor.fetchone returns None, so TypeError
            queryset = Unique.objects.create(domain=domain, number=1)
            number = 1        
                
        return number

def checkunique(domain, receivednumber):
    ...
    if newnumber  == receivednumber:
        ...
        if botsglobal.ini.getboolean('acceptance','runacceptancetest',False):
            ...
            queryset = Unique.objects.filter(domain=domain).update(number=newnumber-1)
...
def sendbotsemail(partner,subject,reporttext):
    ...
    mails = PartnerMail.objects.filter(partner=partner).values_list('mail')
    ccs = PartnerMailCC.objects.filter(partner=partner).values_list('cc')
    addresses = merge(mails,ccs)
    for address in addresses:
        if address:
            recipient_list = ','.join(recipient_list,address)
    try:
...
def prepare_confirmrules():
    ...
    listconfirmdict = list(ConfirmRule.objects.filter(active=True).order_by('negativerule').values(
        'idchannel',
        'frompartner',
        'topartner',
        'confirmtype',
    ...
    for confirmdict in listconfirmdict:
        botsglobal.confirmrules.append(dict(confirmdict))
...
def set_asked_confirmrules(routedict,rootidta):
    ...
    for row in queryset:
        ...
        if not checkconfirmrules(confirmtype,idroute=routedict['idroute'],idchannel=routedict['tochannel'],
            ...
            continue
        Transaction.object.filter(idta=row['parent']).update(confirmasked=True,confirmtype=confirmtype)
        # changeq('''UPDATE ta
...
def set_database_lock():
    try:
        ts = str(python_datetime.datetime.now())
        print(ts)
        Mutex.objects.update_or_create(mutexk=1,mutexer=0,ts=ts,defaults={'mutexk':'1','mutexer':'0','ts':ts})
        # changeq('''INSERT INTO "mutex" ("mutexk", "mutexer", "ts") VALUES ("1","0","%(ts)s")''',{'ts': ts})
    except:
        return False
    return True

def remove_database_lock():
    Mutex.objects.filter(mutexk=1).delete()
...
def trace_origin(ta,where=None):
    ...
    def get_parent(ta):
        ...
        if ta.parent:   #parent via the normal parent-attribute
            ...
            rows = Transaction.objects.filter(idta__gt=ta.script,idta__lt=ta.idta,child=ta.idta).order_by('idta').values('idta')
            for row in rows:
            # for row in query('''SELECT idta
            #                      FROM ta
