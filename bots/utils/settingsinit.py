...
#bots-modules
from bots.utils import settingslib, botsglobal
...
class BotsConfig(ConfigParser.RawConfigParser):
    ...
    def get(self,section, option, default='', **kwargs):
        if self.has_option(section, option):
            ...
            raise settingslib.BotsError('No entry "%(option)s" in section "%(section)s" in "bots.ini".',{'option':option,'section':section})
        else:
            return default
...
def generalinit(configdir):
    ...
    try:                        #first check if is configdir outside bots-directory: import configdir.settings.py
        # we expect that the project nae is equal to the main config dir
        PROJ_NAME = Path(__file__).resolve().parent.parent.parent.parts[-1]
        importnameforsettings = os.path.normpath(os.path.join(PROJ_NAME,'settings')).replace(os.sep,'.')
        settings = settingslib.botsbaseimport(importnameforsettings)
        ...
        try:
            ...
            if not os.path.exists(configdir):    #check if configdir exists.
                raise settingslib.PanicError('In initilisation: path to configuration does not exists: "%(configdir)s".',{'configdir':configdir})
    ...
    try:                        #usersys outside bots-directory: import usersys
        ...
        importedusersys = settingslib.botsbaseimport(importnameforusersys)
        ...
        try:
            ...
            if not os.path.exists(usersys):    #check if configdir exists.
                raise settingslib.PanicError('In initilisation: path to configuration does not exists: "%(usersys)s".',{'usersys':usersys})
    ...
    botsglobal.ini.set('directories','templatehtml',settingslib.join(botsglobal.ini.get('directories','usersysabs'),'grammars/templatehtml/templates'))
    ############################################################################
    #Botssys####################################################################
    ...
    botsglobal.ini.set('directories','botssys',settingslib.join(botssys))   #use absolute path
    botsglobal.ini.set('directories','data',settingslib.join(botssys,'data'))
    botsglobal.ini.set('directories','logging',settingslib.join(botssys,'logging'))
    ############################################################################
    #other inits##############################################################
    ...
    settingslib.dirshouldbethere(botsglobal.ini.get('directories','data'))
    settingslib.dirshouldbethere(botsglobal.ini.get('directories','logging'))
    initbotscharsets()  #initialise bots charsets
    #node.Node.checklevel = botsglobal.ini.getint('settings','get_checklevel',1)
    settingslib.settimeout(botsglobal.ini.getint('settings','globaltimeout',10))
    ############################################################################
    #Init django#################################################################################
...
def codec_search_function(encoding):
    try:
        module,filename = settingslib.botsimport('charsets',encoding)
    except settingslib.BotsImportError:         #charsetscript not there; other errors like syntax errors are not catched
        return None
    else:
...
def initenginelogging(logname):
    ...
    if botsglobal.ini.get('settings','log_when',None) == 'daily':
        handler = logging.handlers.TimedRotatingFileHandler(settingslib.join(botsglobal.ini.get('directories','logging'),logname+'.log'),when='midnight',backupCount=botsglobal.ini.getint('settings','log_file_number',10))
    else:
        handler = logging.handlers.RotatingFileHandler(settingslib.join(botsglobal.ini.get('directories','logging'),logname+'.log'),backupCount=botsglobal.ini.getint('settings','log_file_number',10))
