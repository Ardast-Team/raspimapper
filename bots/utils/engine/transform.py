...
#*******************************************************************************************************************
from ..botslib import addinfo,updateinfo,changestatustinfo,checkunique,sendbotsemail,strftime
...
def translate(startstatus,endstatus,routedict,rootidta):
    ...
    rows = Transaction.objects.filter(idta__gt=rootidta,
                                      status=startstatus,
                                      statust=OK,
                                      idroute=routedict['idroute']).order_by('idta').values('idta',
                                                                                            'frompartner',
                                                                                            'topartner',
                                                                                            'filename',
                                                                                            'messagetype',
...
def persist_add(domain,botskey,value):
    ''' store persistent values in db.
    '''
    content = pickle.dumps(value,0).decode('iso-8859-1')
    try:
        Persist.objects.create(domain=domain,botskey=botskey,content=content)
        ...
        raise botslib.PersistError('Failed to add for domain "%(domain)s", botskey "%(botskey)s", value "%(value)s".',
                                    {'domain':domain,'botskey':botskey,'value':value})

def persist_update(domain,botskey,value):
    ''' store persistent values in db.
    '''
    content = pickle.dumps(value,0).decode('iso-8859-1')
    Persist.objects.filter(domain=domain,botskey=botskey).update(content=content,ts=strftime('%Y-%m-%d %H:%M:%S'))
...
def persist_add_update(domain,botskey,value):
    # add the record, or update it if already there.
    try:
        persist_add(domain,botskey,value)
    except:
        persist_update(domain,botskey,value)

def persist_delete(domain,botskey):
    ''' store persistent values in db.
    '''
    Persist.objects.filter(domain=domain,botskey=botskey).delete()
    # botslib.changeq(''' DELETE FROM persist
    #                     WHERE domein=%(domein)s
    #                     AND botskey=%(botskey)s''',
    #                     {'domein':domein,'botskey':botskey})

def persist_lookup(domain,botskey):
    ''' lookup persistent values in db.
    '''
    rows = Persist.objects.filter(domain=domain,botskey=botskey).values('content')
...
def ccode(ccodeid,leftcode,field='rightcode',safe=False):
    ...
    '''
    rows = Ccode.objects.filter(ccodeid_id=ccodeid,leftcode=leftcode).values(field)
...
def reverse_ccode(ccodeid,rightcode,field='leftcode',safe=False):
    ...
    rows = Ccode.objects.filter(ccodeid_id=ccodeid,rightcode=rightcode).values(field)
    # for row in botslib.query('''SELECT ''' +field+ '''
    #                             FROM ccode
    #                             WHERE ccodeid_id = %(ccodeid)s
...
def getcodeset(ccodeid,leftcode,field='rightcode'):
    ...
    '''
    terug = []
    rows = Ccode.objects.filter(ccodeid_id=ccodeid,leftcode=leftcode).order_by('id').values(field)
    # for row in botslib.query('''SELECT ''' +field+ '''
    #                             FROM ccode
    #                             WHERE ccodeid_id = %(ccodeid)s
    #                             AND leftcode = %(leftcode)s
...
def partnerlookup(value,field,field_where_value_is_searched='idpartner',safe=False):
    ...
    '''
    rows = Partner.objects.filter({field_where_value_is_searched : value}).values(field)
    # for row in botslib.query('''SELECT ''' +field+ '''
    #                             FROM partner
