    ...
    for row in botslib.query('''SELECT editype,messagetype,envelope,rsrv3,frompartner,topartner,testindicator,charset,rsrv5,sum(nrmessages) as nrmessages
        ...
        try:
            ...
            for row2 in botslib.query('''SELECT idta,filename
                                        ...
                                        AND (testindicator=%(testindicator)s OR testindicator IS NULL)
                                        AND envelope=%(envelope)s
                                        AND (rsrv3=%(rsrv3)s OR rsrv3 IS NULL)
...
class x12(Envelope):
    ...
    def run(self):
        ...
        self.ta_info['reference'] = self.envelope_content[0].get('ISA13') or str(botslib.unique('isacounter_' + self.ta_info['topartner'] if botsglobal.ini.getboolean('settings','interchangecontrolperpartner',False) else self.ta_info['frompartner']))
        #date and time
        senddate = botslib.strftime('%Y%m%d')
