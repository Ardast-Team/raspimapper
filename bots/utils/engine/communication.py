    ...
    def outcommunicate(self):
        ...
        for row in botslib.query('''SELECT idta,filename,numberofresends
            ...
            try:    #for each db-ta:
                ...
                txt = botslib.txtexc()
                ta_to.update(statust=ERROR,errortext=txt,numberofresends=int(0 if row['numberofresends'] is None else row['numberofresends'])+1)
            else:
                ta_to.update(statust=DONE,filename=tofilename,numberofresends=int(0 if row['numberofresends'] is None else row['numberofresends'])+1)
            finally:
                ta_from.update(statust=DONE)
    ...
    def outcommunicate(self):
        ...
        for row in botslib.query('''SELECT idta,filename,numberofresends
            ...
            try:    #for each db-ta:
                ...
                txt = botslib.txtexc()
                ta_to.update(statust=ERROR,errortext=txt,numberofresends=int(row['numberofresends'])+1)
            else:
                ta_to.update(statust=DONE,filename=tofilename,numberofresends=int(row['numberofresends'])+1)
            finally:
                ta_from.update(statust=DONE)
